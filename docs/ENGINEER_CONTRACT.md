...
The Git boundary applies prospectively. Earlier explicitly authorized work,
including F0 commit `079b6a5483286bda41a56e3c2f8e117138270bb2`, is not a
violation and must not be amended or rewritten merely to adopt this contract.

## 2. Source of truth

Use [docs/README.md](README.md) for canonical ownership: PROJECT_DESIGN.md
owns product direction; architecture/ and ADRs own technical decisions; features/
own business contracts; milestones/ own authorized scope. These collaboration
documents do not redefine them. Source code/tests are executable evidence, not
automatic approval of behavior that contradicts an accepted contract. Accepted
engineering notes preserve reusable knowledge and reference its owning documents.
...
Follow the read order in [Engineer Workflow](ENGINEER_WORKFLOW.md#2-discover-authority-before-implementation), as indexed by [the documentation map](README.md).
...
- complete changed files at exact repository-relative paths as the default deliverable;
...
merge, tag, release and deployment are separate permissions.
