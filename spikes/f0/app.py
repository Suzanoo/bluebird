...
def generate(raw, method):
    ...
    try:
        try:
            data, _ = convert(raw, method)
            if monotonic() - started > TARGET_SECONDS:
                return error(504, 'timeout', 'Conversion exceeded the proof time target.')
            ...
            return Response(data, media_type=XLSX_TYPE, headers={
                'Content-Disposition': 'attachment; filename="progress-f0.xlsx"',
...
async def convert_xml(request: Request):
    ...
    return await run_in_threadpool(generate, bytes(raw), methods[0])
